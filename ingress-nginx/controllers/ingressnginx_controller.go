...
var _ reconcile.Reconciler = &IngressNginxReconciler{}

// IngressNginxReconciler reconciles a IngressNginx object
type IngressNginxReconciler struct {
	client.Client
	Log    logr.Logger
...
func (r *IngressNginxReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	labels := map[string]string{
		"k8s-app": "ingressnginx",
	}

	watchLabels := declarative.SourceLabel(mgr.GetScheme())

	if err := r.Reconciler.Init(mgr, &api.IngressNginx{},
	...
	c, err := controller.New("ingressnginx-controller", mgr, controller.Options{Reconciler: r})
	if err != nil {
		return err
	}

	// Watch for changes to IngressNginx
	err = c.Watch(&source.Kind{Type: &api.IngressNginx{}}, &handler.EnqueueRequestForObject{})
	if err != nil {
		return err
	}
